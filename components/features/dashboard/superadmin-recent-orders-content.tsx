...
import { SuperAdminRecentOrdersError } from "./superadmin-recent-orders-error";
import { SuperAdminRecentOrdersSkeleton } from "./superadmin-recent-orders-skeleton";
...
export function SuperAdminRecentOrdersContent() {
  const {
    data: recentOrders,
    isLoading,
    isFetching,
    ...
    queryFn: AdminDashboardApi.getOrders,
  });

  const t = useTranslations("Dashboard.superadmin.recentOrders");

  if (isLoading || isFetching) {
    return <SuperAdminRecentOrdersSkeleton />;
  }

  if (isError) {
    return (
      <SuperAdminRecentOrdersError
